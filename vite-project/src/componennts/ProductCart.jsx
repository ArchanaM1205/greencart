...
const ProductCart = ({ product }) => {
  ...
  const { currency, addToCart, removedFromCart, cartItems } = useAppContext();
  ...
  const handleClick = () => {
    // ✅ Navigate to product detail (you can update the path format if needed)
        ...
        <div className="flex items-center gap-0.5 mt-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <img
              ...
              src={rating >= i + 1 ? assets.star_icon : assets.star_dull_icon}
              alt="star"
            />
          ))}
          <p className="ml-1 text-xs">{rating.toFixed(1)}</p>
        </div>
        ...
        <div className="flex items-end justify-between mt-3">
          ...
          <div onClick={(e) => e.stopPropagation()} className="text-indigo-600">
            {count === 0 ? (
              ...
              <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-indigo-500/20 rounded select-none">
                <button
                  onClick={() => removedFromCart(product._id)}
